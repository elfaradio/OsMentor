...
export const solveSRTF = (inputProcesses) => {
    ...
    while (completedQueue.length < processes.length || cpuProcess) {
        ...
        // 2. Preemption check: put currently running process back into ready queue to evaluate
        if (cpuProcess) {
        ...
        // 3. Sort ready queue for SRTF: by remainingTime, then arrivalTime, then ID
        readyQueue.sort((a, b) => {
        ...
        // 4. Schedule the process with the shortest remaining time
