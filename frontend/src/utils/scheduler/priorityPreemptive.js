...
export const solvePriorityPreemptive = (inputProcesses) => {
    ...
    while (completedQueue.length < processes.length || cpuProcess) {
        ...
        // 2. Preemption evaluation: put running process back to ready queue
        if (cpuProcess) {
        ...
        // 3. Sort ready queue: by priority (lower value = higher priority), then arrivalTime, then ID
        readyQueue.sort((a, b) => {
        ...
        // 4. Schedule process with highest priority (lowest priority number)
